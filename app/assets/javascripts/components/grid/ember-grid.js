...
GRID.PaginatedMixin = Ember.Mixin.create({
    ...
    update: function(page, limit, query)
    {
        ...
        var fullUrl = endpoint + '?page=' + page + '&items=' + limit + addSort +addParmas+temp;
        ...
        xhr.then(function(data){
            ...
            {
                data[collection][i]['num'] = (page*limit-limit)+i+1;
    ...
    hasNextPage: function(){
        var page = this.get('meta.page');
        var pages = this.get('meta.pages');
        return page < pages;
...
GRID.BodyView = Ember.CollectionView.extend({
    ...
    emptyView: Ember.View.extend({
        tagName: 'tr',
        template: Ember.Handlebars.compile('<td {{bind-attr colspan="controller.columns.length"}} class="muted">אין נתונים להצגה.</td>')
    })
});
...
GRID.PageListView = Ember.ContainerView.extend({
    ...
    actions: {
        ...
        'firstPage': function () {
            ...
            this.get('controller').send("firstPage");
        },
        ...
        'lastPage': function () {
            ...
            this.get('controller').send("lastPage");
        },
...
GRID.Filter = Ember.View.extend({
    tagName: 'div',
    classNames: ['search'],
    defaultTemplate: Ember.Handlebars.compile('{{#unless hidesearch}}{{view Ember.TextField class="search-query input-medium search hidden-xs" placeholder="חיפוש..." valueBinding="query"}}{{/unless}}')
});
